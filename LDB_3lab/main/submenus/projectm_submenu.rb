...
def projm_submenu
  loop do
    ...
    when 'Add member'
      ...
      project = $project_manager.load_project(proj.split(':').first)

      project.add_member($prompt.ask('Member mail:'))
    ...
    when 'Remove member'
      ...
      project = $project_manager.load_project(proj.split(':').first)
      project.remove_member($prompt.ask('Member mail:'))
      $project_manager.delete_project(Project.new(num: proj.split(':').first))
      $project_manager.save_project(project)
    ...
    when 'Set status'
      ...
      choice = $prompt.select('', %w[Edit Back], cycle: true)
      if choice.eql?('Edit')
        if project.parm_project_status($prompt.ask('New status: ', default: ' '))
