...
def wgm_submenu
  loop do
    ...
    when 'Add member'
      ...
      group = $workgroup_manager.load_group(gr.split(':').first)
      group.add_group_member(User.new(email: $prompt.ask('Member mail:')))
      $workgroup_manager.save_group(group)
    ...
    when 'Remove member'
      ...
      group = $workgroup_manager.load_group(gr.split(':').first)
      group.remove_group_member(User.new(email: $prompt.ask('Member mail:')))
      $workgroup_manager.save_group(group)
    ...
    when 'Add task'
      ...
      group = $workgroup_manager.load_group(gr.split(':').first)
      group.add_group_task($prompt.ask('Task: ', default: ' '))
      $workgroup_manager.save_group(group)
    ...
    when 'Remove task'
      ...
      group = $workgroup_manager.load_group(gr.split(':').first)
      group.remove_group_task($prompt.ask('Task: ', default: ' '))
      $workgroup_manager.save_group(group)
